...
const SearchBar = ({onSubmit, onReset, placeHolder}: Props) => {
    ...
    return(
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={`flex items-center flex-1 gap-3 justify-between flex-row border-2 rounded-full p-3 m-5 ${form.formState.errors.searchQuery && "border-red-500"}`}>
                ...
                <FormField control={form.control} name="searchQuery" render={({field}) => (<FormItem className="flex-1">
                    <FormControl>
                        <Input {...field} className="border-none shadow-none text-xl focus-visible:ring0" placeholder={placeHolder}/>
                    </FormControl>
                </FormItem>)}/>
